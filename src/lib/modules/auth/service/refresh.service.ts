...
export async function rotateRefreshToken(oldTokenId: string, newPlainToken: string, userId: string) {
    ...
    await prisma.$transaction(async (tx: { refreshToken: { update: (arg0: { where: { id: string; }; data: { revoked: boolean; }; }) => any; create: (arg0: { data: { userId: string; tokenHash: string; expiresAt: Date; rotatedFrom: string; }; }) => any; }; }) => {
